...
public sealed class AppFixture : IDisposable
{
    ...
    public AppFixture()
    {
        ...
        var login = App.GetMainWindow(Automation, TimeSpan.FromSeconds(30));
        login.FindFirstDescendant(cf => cf.ByAutomationId("SignIn"))!.AsButton().Invoke();

        Window = Retry.WhileNull(
            () => App.GetAllTopLevelWindows(Automation).FirstOrDefault(w => w.Title.Contains("Solitaire Desk")),
            TimeSpan.FromSeconds(30)).Result
            ?? throw new InvalidOperationException("Main window never appeared — is DiamondApi running on :5000?");
